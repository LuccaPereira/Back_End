...
import * as admin from "firebase-admin"; //admin.firestore.QuerySnapshot<admin.firestore.DocumentData>
import { Payment } from "./structurs";
import { Analyze, GenerateTxIdStr, GetRandomIntInclusive, MensagemErro } from "./controller";
...
export const addPagamento = functions
  ...
  .https.onCall(async (PaymentData: Payment): Promise<Payment> => {
    ...
    const errorCode = Analyze(PaymentData.valor, PaymentData.qtdeTempo, PaymentData.placa, PaymentData.pix);

    const errorMessage = MensagemErro(errorCode);
    if (errorCode > 0) {
      result = {
        placa: "Unlucky",
        qtdeTempo: "ERROR",
